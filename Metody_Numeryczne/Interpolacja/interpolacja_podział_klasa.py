...
class Interpolacja:
    def __init__(self, df_all):
        self.df_all = df_all
        self.df_nodes = None
        self.df_to_calc = None
        self.df_interpolation = None
        self.intervals = len(self.df_nodes) // MAX_NUMBER_OF_POINTS

         # df_nodes - tylko węzły
        self.df_nodes = self.df_all.dropna()
        self.df_nodes.reset_index(inplace=True, drop=True)

        self.df_all['F(x)'] = np.nan

    
    # Wyznaczenie tablicy ilorazów różnicowych
    def ilorazy_roznicowe(self, df_nodes):
