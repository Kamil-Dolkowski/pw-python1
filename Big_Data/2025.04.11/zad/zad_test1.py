...
def join_images(tiles_pixels, images_directory, tSize = 150):
    ...
    begin = time.time()
    new_image.save(f"final_img.png", format="PNG", compress_level=1)
    end = time.time()
    print(f"100% [{(end-begin):.1f}s]")
...
def main():
    images_directory = "/home/u334531/Pobrane/VanillaDefault 1.21.5/assets/minecraft/textures/block"

    begin = time.time()
    calc_avg_rgb_for_images(images_directory)

    tiles_pixels = main_image_to_tiles("minecraft.png", 1)

    join_images(tiles_pixels, images_directory, tSize = 16)
    end = time.time()
